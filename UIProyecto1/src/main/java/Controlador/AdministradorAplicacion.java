    ...
    public ArrayList<Vehiculo> getVehiculosTipo(TEstilo tipo, Calendar Desde, Calendar Hasta, boolean ConSede) {
        ...
        if (ConSede) {
            for (Vehiculo V: listaVehiculos) {
                if (V.getEstado().equals(TEstado.Activo) && V.getEstilo().equals(tipo) && !V.isRentadoEnElRango(Desde, Hasta) && V.getSede().equals(Vista.RealizarReserva.TextFieldSedeRecogida.getSelectedItem())) {
                    temp.add(V);
                }
            }
            return temp;
